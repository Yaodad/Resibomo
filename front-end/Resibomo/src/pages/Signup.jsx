    ...
    <div className='flex flex-col justify-center items-center w-full h-screen'>
        ...
        <div className='sm:w-[500px] w-[310px] mx-auto px-[36px] py-[8px] pb-6 bg-white shadow-lg'>
            <div className='flex justify-between items-center pb-3'>
                ...
                <h1 className=' sm:text-[24px] text-[16px] font-bold text-tertiary tracking-tighter'>Sign up</h1>
            </div>
            <form className='flex flex-col gap-1 text-primary font-sans' onSubmit={handleSubmit}>
                ...
                <button type="submit" disabled={isLoading} className='rounded-md bg-tertiary font-primary text-[14px] text-white p-2 w-full hover:bg-white hover:text-tertiary hover:border-tertiary border-tertiary border-[1px] transition-all duration-500 ease-out'>Sign up</button>
                {error && <div className='w-full mt-3 p-2 rounded-md bg-[#f8d7da] border-[1px] border-[#f1aeb5] text-[#58151c] text-sm'>{error}</div>}
            </form>
            <p className='text-[14px] text-center pt-5'>Already have an account? <Link to='/login'><span className='text-tertiary hover:underline'>Log in!</span></Link></p>
        </div>
    </div>
  )
}
